...
import ky from "ky";
import { deleteEmptyHeaders, toCurl } from "fetch-addons";
...
function createHmac(secret, data) {
  var _a;
  if (!((_a = crypto) == null ? void 0 : _a.createHmac))
...
Debug("magicbell:info");
const debug = Debug("magicbell:debug");
const error = Debug("magicbell:error");
function mask(str) {
  return str;
}
...
function createListener(client, args = {}) {
  ...
  async function connect(options) {
    ...
    eventSource.addEventListener("message", (event) => {
      if (event.origin && event.origin !== sseHost)
        return;
